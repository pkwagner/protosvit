# From funny birds: https://github.com/visinf/funnybirds-framework?tab=readme-ov-file
...
def accuracy_protocol(model, data_path, batch_size):
    class Summary(Enum):
        NONE = 0
        AVERAGE = 1
        SUM = 2
    ...
    test_dataset = FunnyBirdsDataset(
        data_path, "test", transform=transforms, eval_funny_birds=True
    )
...
def controlled_synthetic_data_check_protocol(model, explainer, data_path, nr_itrs=2501):
    transforms = None
    ...
    for samples in tqdm(test_loader):
        ...
        if nr_itrs == number_valid_samples:
            break
...
def single_deletion_protocol(model, explainer, data_path, nr_itrs=2501):
    transforms = None
...
def preservation_check_protocol(model, explainer, data_path, nr_itrs=2501):
    transforms = None
    ...
    for samples in tqdm(test_loader):
        ...
        part_maps = part_maps.cuda(non_blocking=True)
...
def deletion_check_protocol(model, explainer, data_path, nr_itrs=2501):
    transforms = None
...
def target_sensitivity_protocol(model, explainer, data_path, nr_itrs=2501):
    ...
    for sample in tqdm(test_loader):
        ...
        part_map = part_map.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        output = model(image)
        ...
        class1 = torch.tensor([class1]).cuda(non_blocking=True)
        class2 = torch.tensor([class2]).cuda(non_blocking=True)
...
def distractibility_protocol(model, explainer, data_path, nr_itrs=2501):
    transforms = None
    ...
    for sample in tqdm(test_loader):
        ...
        for score_key in score.keys():
            ...
            if (
                ...
                or original_score < score[score_key]
            ):  # second condition: removing the part increases the class evidence --> part is not important
...
def background_independence_protocol(model, data_path, nr_itrs=2501):
    transforms = None
